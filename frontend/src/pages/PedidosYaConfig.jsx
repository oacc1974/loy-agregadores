...
const PedidosYaConfig = () => {
  ...
  const loadConfig = async () => {
    try {
      const response = await pedidosyaService.getConfig();
      if (response.data) {
        setConfig({
          restaurantId: response.data.credentials.restaurantId,
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await pedidosyaService.configure(config);
  ...
  const handleTestConnection = async () => {
    setTesting(true);
    try {
      const response = await pedidosyaService.testConnection();
      if (response.success) {
        alert('✅ Conexión exitosa con PedidosYa');
      } else {
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-3xl font-bold">Configuración de PedidosYa</h1>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configura las credenciales de tu cuenta de PedidosYa
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium">Restaurant ID</label>
              <Input
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Client ID de la API"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Client Secret de la API"
              />
...
export default PedidosYaConfig;
