...
const UberConfig = () => {
  ...
  const loadConfig = async () => {
    try {
      const response = await uberService.getConfig();
      if (response.data) {
        setConfig({
          storeId: response.data.credentials.storeId,
          clientId: response.data.credentials.clientId,
          clientSecret: '', // No mostrar el secret
          settings: response.data.settings,
        });
      }
    } catch (error) {
      // Config no existe aún
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await uberService.configure(config);
  ...
  const handleTestConnection = async () => {
    setTesting(true);
    try {
      const response = await uberService.testConnection();
      if (response.success) {
        alert('✅ Conexión exitosa con Uber Eats');
      } else {
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-3xl font-bold">Configuración de Uber Eats</h1>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configura las credenciales de tu cuenta de Uber Eats
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="ID de tu tienda en Uber Eats"
              />
            </div>
